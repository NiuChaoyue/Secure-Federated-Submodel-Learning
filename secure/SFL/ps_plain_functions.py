...
def update_global_model(round_num, communication, clients, dataset_info, batches_info_dict, hyperparameters, \
    ...
    userIDs_dict = dict()   # client_index -> userID
    global_perturbed_itemIDs = []
    global_perturbed_cateIDs = []
    any_client_flag = True
    for client_index, client_ids_info in batches_info_dict.items():
        userIDs_dict[client_index] = client_ids_info['userID'][0]
        if any_client_flag:
            global_perturbed_itemIDs = client_ids_info['perturbed_itemIDs']
            global_perturbed_cateIDs = client_ids_info['perturbed_cateIDs']
    ...
    gathered_weighted_delta_submodel, gathered_userIDs_count, gathered_itemIDs_count, gathered_cateIDs_count, \
       gathered_other_count = ps_sfsa.server_side_secure_federated_submodel_averaging(communication, clients, \
                  dataset_info, userIDs_dict, global_perturbed_itemIDs, global_perturbed_cateIDs, global_model_shape, \
